...
file='bcc-csm1-1_changeH2Otime'   # 'bcc-csm1-1'  'bcc-csm1-1_changeH2Otime'

point='1032'                      # '657'<-'六腳'  '955'<-'旗山'  '1032'<-新園

year = '2030'                     # '2030'  '2040'  '2050'

stress_num = 6                    # 資料為0-11，設定大於多少判定為stress
...
ggplot(merge)+
  geom_line(aes(x=Date,y=CLIMAT4*2.54),size=1)+
  labs(x="Date",y="Precipitation (mm/day)",title=t)+theme_bw()+

  theme(plot.title=element_text(hjust = 0.5,face="bold",size=15))+

  geom_text(aes(x=wDate,label="↓",y=2.3),color="blue",size = 15)+
...
ggplot(merge)+geom_line(aes(x=Date, y=LAREAT))+labs(x='Date',y="Leaf area (cm/plant)",title=t)+theme_bw()+
  ...
  geom_text(aes(x=wDate,label="↓",y=440),color="blue",size = 15)+  
